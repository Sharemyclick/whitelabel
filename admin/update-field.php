...
//-----------------------------------------------------------------------------------// 
        //UPDATE FORM
        $bdd->exec(' UPDATE fields SET type="'.$_POST['type'].'" , label="'.$_POST['label'].'" WHERE id='.$id_field);
//---------------------------------------------------------------------------------------//
			...
			<div class="widgetcontent">
                ...
                <div class="widgetcontent bordered shadowed nopadding">
                    <form name="form_field" class="stdform stdform2" method="post" action="" enctype="multipart/form-data">
                         ...
                         <?php
                            $reqField = $bdd->query('SELECT * FROM fields WHERE id='.$id_field) or die(print_r($bdd->errorInfo())); 
                        ...
                        <p>
                            ...
                            <span class="field">
                                <select  name="type" id="field_type_id" class="status" >
                                    <option value=""></option>
                                    <?php 
                                        $reqFieldType = $bdd->query("SELECT * FROM field_type");
                                        while ($field_type = $reqFieldType->fetch())
                                        {
                                            ?>
                                    <option value="<?php echo $field_type['name']; ?>" <?php if($field_type['name']===$field['type']){echo 'selected';} ?>> <?php echo $field_type['name']; ?> </option>
                                            <?php
                                        }
                                    ?>
                                </select>
                        ...
                        <p>
                            <label>Fields *</label>
                            <span class="field"><input type="text" name="label" class="input-xxlarge" required="required" value="<?php echo $field['label']; ?>" /></span>
