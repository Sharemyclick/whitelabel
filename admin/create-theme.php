...
//---------------------- INSERT THEMES ---------------------------------------------------------------------
        ...
        'VALUES (:name, :image, :picto, :status, :admin_id)');
// We execute the request by transmitting the parameter list
	$reqThemes->execute(array(
		'name' => $_POST['name'],
                ...
                $id_themes = $bdd->lastInsertId();
                // The request processing is terminated
                $reqThemes->closeCursor();
                         
//---------------------- INSERT TEMPLATE_THEMES ---------------------------------------------------------------------
        $reqThemesTemplate = $bdd->prepare('INSERT INTO template_themes (url, meta, title, text, alt, link, h1, themes_id)'.
        'VALUES (:url, :meta, :title, :text, :alt, :link, :h1, :themes_id)');
// We execute the request by transmitting the parameter list
