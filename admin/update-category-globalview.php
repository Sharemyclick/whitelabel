...
if(isset($_POST) && !empty($_POST)){
    foreach($_POST as $indPost => $valPost){
        ...
        {
            $bdd->exec('DELETE FROM categories WHERE id='.$_POST['id']);
            $bdd->exec('DELETE FROM template_categories WHERE categories_id='.$_POST['id']); 
        }
    }
}
...
<body>
    ...
    <div class="rightpanel">
      ...
      <div class="pagetitle">
        	<h1>Modify Theme</h1> <span><strong><?php echo ucfirst($_SESSION['login']); ?></strong> , please choose the theme to modify. </span>
        </div><!--pagetitle-->
        ...
        <div class="maincontent">
            ...
            <table class="table table-bordered" id="dbase">
					...
					while ($category = $reqCategories->fetch())
						{?>
                        <tr><form class="stdform" method="post" action="update-category.php?id=<?php echo $category['id'];?>">

                            <td class="centeralign" >
                                <a href="update-category.php?id=<?php echo $category['id']; ?>" ><p align="center"> <?php echo $category['name'] ?> </p></a>
                            </td>
                            
                             <td class="centeralign" >
                                                        <p align="center">   <?php $reqParentCategory = $bdd->query('SELECT * FROM categories WHERE id='.$category['parent_id']) or die(print_r($bdd->errorInfo())); // On traque l'erreur s'il y en a une
                                                         while ($parentCategory = $reqParentCategory->fetch())echo $parentCategory['name'] ?> </p>
                            </td>
                            
                            
                            <td>
                                <p align="center"> <img  src="<?php echo 'http://localhost/white_label/admin/img/categories/'.$category['image'] ?>" height="92" width="52" > </p>
                            </td>
                            
                            <td>
                                <p align="center"> <img  src="<?php echo 'http://localhost/white_label/admin/img/categories/'.$category['picto'] ?>" height="92" width="52" ></p>
                            </td>
                            ...
                            <td class="centeralign" style="text-align: left">
                                 <p align="center">
                                <?php 
                                $reaThemes = $bdd->query('SELECT * FROM themes WHERE id='.$category['themes_id']) or die(print_r($bdd->errorInfo())); // it tracks error if there is one
                                
                                while ($theme= $reaThemes->fetch())
						{
                                                    echo $theme['name'];
                                                }
                                ?></p>
                            </td>
                            <td class="centeralign"> <a href="update-category.php?id=<?php echo $category['id'] ?>" ><input type="button" class="btn btn-success" name="update" value="Update"> </a>
                            </td>
                          <!--'LEFT JOIN answers_questions ON answers_questions.questions_id=questions.id LEFT JOIN answers ON answers.id=answers_questions.answers_id '-->
                            ...
                            <td class="centeralign" >
                                <form  id="form-delete-<?php echo $category['id']; ?>" name="form-delete-<?php echo $category['id']; ?>" method="post" action="">
                                  <a href="#"  class="deleterowcustomized"><span id="delete<?php echo $category['id']; ?>" class="icon-trash"></span></a>
                                  <input type="hidden" name="hidden_delete<?php echo $category['id']; ?>" value="" >
