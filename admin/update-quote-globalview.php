...
if(isset($_POST) && !empty($_POST)){
    foreach($_POST as $indPost => $valPost){
        ...
        {
            $bdd->exec('DELETE FROM devis WHERE id='.$_POST['id']);
            $bdd->exec('DELETE FROM domain_devis WHERE devis_id='.$_POST['id']); 
            $bdd->exec('DELETE FROM form_devis WHERE devis_id='.$_POST['id']); 
            $bdd->exec('DELETE FROM devis_admin WHERE devis_id='.$_POST['id']); 
        }
    }
}
            ...
            <table class="table table-bordered" id="dbase">
					...
					while ($admin = $reqAdmin->fetch())
						{?>
                            ...
                            <td class="centeralign" >
                                <form  id="form-delete-<?php echo $quote['id']; ?>" name="form-delete-<?php echo $quote['id']; ?>" method="post" action="">
                                  <a href="#"  class="deleterowcustomized"><span id="delete<?php echo $quote['id']; ?>" class="icon-trash"></span></a>
                                  <input type="hidden" name="hidden_delete<?php echo $quote['id']; ?>" value="" >
